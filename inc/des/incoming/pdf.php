...
<head>
    ...
    <title data-i18n=" incoming"> incoming</title>
    ...
    <table class="table table2 border-top m-0 " style="width: 100%;" >
      <thead>
         <tr>
          ...
          <th class="table2" style="font-size: 12px; padding: 5px 5px 5px 5px; color:black;   font-weight: bold; text-align:center;background-color: #ccc;letter-spacing:0px;"  ><?php if ($_COOKIE['lang']=="ar"){echo 'رقم الوارد';}else if ($_COOKIE['lang']=="en"){  echo'incoming number';} ?></th>

          <th class="table2" style="font-size: 12px; padding: 5px 5px 5px 5px; color:black;   font-weight: bold; text-align:center;background-color: #ccc;letter-spacing:0px;"  ><?php if ($_COOKIE['lang']=="ar"){echo 'تاريخ الورود';}else if ($_COOKIE['lang']=="en"){  echo'incoming date';} ?> </th>
...
$sql->selectjoin("  
  ...
  "
  ,"incoming","
    transaction_type ON incoming.transaction_type_id = transaction_type.id 
    INNER JOIN importance ON incoming.importance_id = importance.id 
    INNER JOIN confidentiality ON incoming.confidentiality_id = confidentiality.id 
    INNER JOIN dialect ON incoming.dialect_id = dialect.id
    INNER JOIN user ON incoming.user_id = user.id
    where
    incoming.status=$status and 
    incoming_date BETWEEN' $date1' AND '$date2'
...
<tr>
          ...
          <td class="table2" style="width:90px;font-size: 12px; padding: 5px 5px 5px 5px; color:black;  font-weight: bold; text-align:center"><?=$row['incoming_date']?></td>
