...
$name_en=filter_var($_POST['name_en'], FILTER_SANITIZE_STRING);
 ...
 $sql->insert('transaction_type',["name"=>"$name","name_en"=>"$name_en"]);
 session_start();
 ...
 $sql->insert('notifications',["user_id"=>"$user_id","page"=>"إضافة نوع معاملة","des"=>"  ﻗﺎﻡ  $name_user ($permissions_user) بإضافة نوع معاملة ","page_en"=>"Add Transaction Type","des_en"=>"$name_user_en ($permissions_user_en) added a transaction type. ","status"=>0,"date"=>"$date_user"]);
