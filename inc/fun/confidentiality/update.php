...
$name_en=filter_var($_POST['name_en'], FILTER_SANITIZE_STRING);
 ...
 $sql->update('confidentiality',["name"=>"$name","name_en"=>"$name_en"],"where id=$id");
 session_start();
 ...
 $sql->insert('notifications',["user_id"=>"$user_id","page"=>"تعديل السرية","des"=>"  ﻗﺎﻡ  $name_user ($permissions_user) بتعديل السرية ","page_en"=>"Edit confidentiality","des_en"=>"$name_user_en   edited a confidentiality. ","status"=>0,"date"=>"$date_user"]);
