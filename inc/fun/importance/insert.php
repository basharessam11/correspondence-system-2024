...
$name_en=filter_var($_POST['name_en'], FILTER_SANITIZE_STRING);
 ...
 $sql->insert('importance',["name"=>"$name","name_en"=>"$name_en"]);
 ...
 $sql->insert('notifications',["user_id"=>"$user_id","page"=>"إضافة أهمية","des"=>"  ﻗﺎﻡ  $name_user ($permissions_user) بإضافة أهمية ","page_en"=>"Add importance","des_en"=>"$name_user_en ($permissions_user_en) added a importance. ","status"=>0,"date"=>"$date_user"]);
